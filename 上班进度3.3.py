...
def update_progress():
    ...
    root.geometry(
        f"{root.winfo_width()}x{root.winfo_height()}+{int(root.winfo_screenwidth() / 2 - root.winfo_width() / 2)}+{int(root.winfo_screenheight() / 2 - root.winfo_height() / 2)}")
    ...
    start_time_entry.grid(row=0, column=1, padx=10, pady=5, sticky='w')
    end_time_label.grid(row=1, column=0, padx=10, pady=5, sticky='w')
    end_time_entry.grid(row=1, column=1, padx=10, pady=5, sticky='w')
    current_time_label.grid(row=2, column=0, padx=10, pady=5, sticky='w')
    current_time_entry.grid(row=2, column=1, padx=10, pady=5, sticky='w')
    auto_button.grid(row=3, column=0, padx=10, pady=10, sticky='w')
    update_button.grid(row=3, column=1, padx=10, sticky='w')
    explanation_label.grid(row=4, column=0, columnspan=2, padx=10, pady=5, sticky='w')
...
end_time_label = tk.Label(main_frame, text="下班时间：")
end_time_entry = ttk.Entry(main_frame, style='TEntry')
end_time_entry.bind('<KeyRelease>', auto_fill_minutes)

current_time_label = tk.Label(main_frame, text="当前时间：")
current_time_entry = ttk.Entry(main_frame, style='TEntry')

explanation_label = tk.Label(main_frame, text="*格式HH:MM")
explanation_label.grid(row=0, column=0, columnspan=2, padx=10, pady=5, sticky='w')

update_button = tk.Button(main_frame, text="开始 (Enter)", command=start_progress)
auto_button = tk.Button(main_frame, text="自动", command=fill_current_time)

start_time_label.grid(row=1, column=0, padx=10, pady=5, sticky='w')
start_time_entry.grid(row=1, column=1, padx=10, pady=5, sticky='w')
end_time_label.grid(row=2, column=0, padx=10, pady=5, sticky='w')
end_time_entry.grid(row=2, column=1, padx=10, pady=5, sticky='w')
current_time_label.grid(row=3, column=0, padx=10, pady=5, sticky='w')
...
root.bind('<Return>', lambda event: start_progress())
